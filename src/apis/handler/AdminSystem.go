...
func AdminSystemL(w http.ResponseWriter, r *http.Request) {
	...
	err = query.AdminSystemL(session, "hlog", "admin", &rep)
	if err != nil {
		WriteError(w, err)
		return
	}
...
func AdminSystemR(w http.ResponseWriter, r *http.Request) {
	...
	err = query.AdminSystemR(session, "hlog", "admin", req.AdminNo, &rep)
	if err != nil {
		WriteError(w, err)
		return
	}
...
func AdminSystemS(w http.ResponseWriter, r *http.Request) {
	...
	err = query.AdminSystemS(session, "hlog", "admin", req.Search, &rep)
	if err != nil {
		WriteError(w, err)
		return
	}
...
func AdminSystemU(w http.ResponseWriter, r *http.Request) {
	...
	err = query.AdminSystemU(session, "hlog", req, &rep)
	if err != nil {
		WriteError(w, err)
		return
	}
